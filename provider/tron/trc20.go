...
func (client *trc20) SendTRC20(wallet provider.Wallet, toAddress string, amount decimal.Decimal, tronApiKey []string) (string, error) {
	...
	response, status, err := jsonRPC([]byte(payload), client.SolidityNodeURL+"/wallet/triggersmartcontract", "POST", tronApiKey)
	if err != nil {
		return "", err
	}
	if status != 200 {
	...
	return client.broadcastTransaction(signedTransaction, tronApiKey)
}

func jsonRPC(data []byte, url string, requestType string, tronApiKey []string) ([]byte, int, error) {
	...
	for i := 0; i < len(tronApiKey); i++ {
		req.Header.Set("TRON-PRO-API-KEY", tronApiKey[i])
...
func (client *trc20) GetTRC20Balance(address string, tronApiKey []string) (decimal.Decimal, error) {
...
func (client *trc20) SendTRX(wallet provider.Wallet, to string, amount decimal.Decimal, tronApiKey []string) (string, error) {
...
func (client *trc20) broadcastTransaction(tx *SignedTransaction, tronApiKey []string) (string, error) {
	js, err := json.Marshal(tx)
	if err != nil {
		return "", err
	}
	response, status, err := jsonRPC(js, client.SolidityNodeURL+"/wallet/broadcasttransaction", "POST", tronApiKey)
	if err != nil {
		return "", err
	}
	if status != 200 {
...
func (client *trc20) GetTRXBalance(address string, tronApiKey []string) (decimal.Decimal, error) {
